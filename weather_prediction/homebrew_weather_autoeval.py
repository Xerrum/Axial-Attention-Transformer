...
def apply_channel_scaling(X, stats):  # X: [B,T,C,H,W] od. [T,H,W,C]
    if X.ndim == 5:  # [B,T,C,H,W]
        mu  = stats["mean"][None,None,:,None,None]  # Broadcast
        std = stats["std"][None,None,:,None,None]
    elif X.ndim == 4:  # [T,H,W,C]
        mu  = stats["mean"][None,None,None,:]  # Broadcast
        std = stats["std"][None,None,None,:]
    else:
        raise ValueError(f"apply_channel_scaling: unerwartete Shape {X.shape} mit {X.ndim} Dimensionen")
...
class ERA5SpatiotemporalDataset(Dataset):
    """Dataset fuer ERA5-Wetterdaten, das Sequenzen fuer Training und Vorhersage bereitstellt."""
    def __init__(
        self,
        data_root: str = None,
        context_len: int = 12,
...
def create_weather_dataloaders(
    ...
    Args:
        ...
        prediction_steps: Anzahl der Vorhersage-Zeitschritte fuer jedes Sample
...
def load_data_for_date_range(start_date, end_date, context_len, prediction_steps, normalize=True, scaler=None):
    ...
    Args:
        ...
        prediction_steps: Anzahl der Vorhersage-Zeitschritte
...
def visualize_predictions_for_wandb(
    ...
    for c, ch in enumerate(channel_names):
        # ---------- Timeseries @ Zentrumspixel ----------
        ci, cj   = H // 2, W // 2
        ts_pred  = pred_plot[:, c, ci, cj]
        ts_true  = targ_plot[:, c, ci, cj]

        fig_ts, ax_ts = plt.subplots(1, 1, figsize=(10, 6))
        ax_ts.plot(dt_times, ts_pred, "-", marker="o", label="Vorhersage @center")
        ax_ts.plot(dt_times, ts_true, "-", marker="x", label="Ground Truth @center")
        ax_ts.set_title(f"Zeitreihe Zentrumspixel – {ch}")
        ax_ts.set_xlabel("Zeit")
        ax_ts.set_ylabel("Wert")
        ax_ts.grid(True)
        ax_ts.legend()
        ...
        # ---------- Heatmaps NUR für Temperatur-Kanäle ----------
        ...
        ax1.set_title(f"Vorhersage {ch} (t=0)")
        ...
        axm1.set_title(f"Vorhersage {ch} (t={mid_t})")
...
def evaluate_from_date(
    ...
    """
    Evaluiert Modellvorhersagen ab einem bestimmten Datum mit jahresübergreifendem Kontext.
    
    Args:
        model: Trainiertes WeatherTransformer-Modell
        start_date: Startdatum für die Vorhersage im Format 'YYYY-MM-DD'
        context_len: Anzahl der Kontext-Zeitschritte
    ...
    Returns:
        dict: Evaluierungsmetriken und Vorhersagedaten
    """
    model.eval()
    ...
    # Vorhersageende (prediction_steps Stunden nach dem Startdatum)
    ...
    # Lade Daten für den gesamten Zeitbereich (Kontext + Vorhersage)
    ...
    # Finde Indizes fuer Kontext- und Vorhersagezeitraum
    ...
    # Stelle sicher, dass wir genuegend Vorhersagedaten haben
    ...
    mse_time_series = [
        {
            ...
            "hours_from_start": i  # Stunden seit Vorhersagebeginn
        }
...
def evaluate_random_dates(
    ...
    prediction_steps=24,
    logger=None,
    # Neue Parameter für vorgeladene Daten
    times=None,
    X_normalized=None,
    lat=None,
    lon=None,
    ...
    """
    Evaluiert Modellvorhersagen für mehrere zufällig ausgewählte Zeitpunkte effizient.
    
    Args:
        model: Trainiertes WeatherTransformer-Modell
        num_samples: Anzahl der zufällig auszuwählenden Zeitpunkte
        year: Jahr, aus dem die Zeitpunkte gewählt werden sollen
        context_len: Anzahl der Kontext-Zeitschritte
        prediction_steps: Anzahl der vorherzusagenden Zeitschritte
        logger: Optional, WandB-Logger
        times: Optional, vorgeladene Zeitstempel
        X_normalized: Optional, vorgeladene und normalisierte Daten
        lat: Optional, vorgeladene Latitude-Daten
        lon: Optional, vorgeladene Longitude-Daten
    
    Returns:
        dict: Zusammengefasste Evaluierungsmetriken und individuelle Ergebnisse
    """
    model.eval()
    device = next(model.parameters()).device
    ...
    if hasattr(model, "encoder_posenc") and hasattr(model.encoder_posenc, "pe"):
        ...
        desired = max(8192, int(context_len))
        if desired > current:
    ...
    # Stelle sicher, dass der Scaler im Modell vorhanden ist
    if not hasattr(model, "scaler") or model.scaler is None:
        raise RuntimeError("Modell hat keinen Scaler. Stelle sicher, dass der Scaler geladen wurde.")
    scaler = model.scaler
    
    # Wenn keine vorgeladenen Daten übergeben wurden, lade sie (ursprüngliches Verhalten)
    if times is None or X_normalized is None or lat is None or lon is None:
        # Bestimme Datenverzeichnis basierend auf dem Jahr
        if year < PREDICTION_YEAR:
            data_dir = os.path.join(os.path.expanduser("~/scratch/era5_data"), "past")
        else:
            data_dir = os.path.expanduser("~/scratch/era5_data")
        
        print(f"[INFO] Suche nach Daten für das Jahr {year} in {data_dir}")
        
        # Lade Daten für das gesamte Jahr (einmalig)
        start_date = datetime(year, 1, 1)
        end_date = datetime(year, 12, 31, 23, 59, 59)
        
        print(f"[INFO] Lade Daten für das gesamte Jahr {year}...")
        
        # Suche verfügbare Dateien
        single_paths = sorted(glob.glob(os.path.join(data_dir, f"era5_single_{year}_*.netcdf")))
        pressure_paths = sorted(glob.glob(os.path.join(data_dir, f"era5_pressure_{year}_*.netcdf")))
        
        if not single_paths or not pressure_paths:
            raise FileNotFoundError(f"Keine Datendateien für das Jahr {year} gefunden")
        
        print(f"[INFO] Gefunden: {len(single_paths)} single-level und {len(pressure_paths)} pressure-level Dateien")
        
        # Lade alle verfügbaren Daten für dieses Jahr
        print("[INFO] Lade alle Daten für das Jahr (dies kann einige Zeit dauern)...")
        times, lat, lon, X = build_stacked_tensor_single_plus_pressure(single_paths, pressure_paths)
        
        print(f"[INFO] Daten geladen: {len(times)} Zeitstempel von {np.datetime_as_string(times[0])} bis {np.datetime_as_string(times[-1])}")
        
        # Normalisiere die Daten
        print("[INFO] Normalisiere Daten...")
        X_normalized = apply_channel_scaling(X, scaler)
    else:
        print(f"[INFO] Verwende vorgeladene Daten mit {len(times)} Zeitstempeln")
        
    # Bestimme gültige Zeitbereiche für Evaluierung
    # - mindestens context_len Stunden vom Anfang entfernt
    # - mindestens prediction_steps Stunden vom Ende entfernt
    min_idx = context_len
    max_idx = len(times) - prediction_steps
    
    if min_idx >= max_idx:
        raise ValueError(f"Nicht genügend Daten für context_len={context_len} und prediction_steps={prediction_steps}")
    
    # Wähle zufällige Indizes für die Evaluierung
    np.random.seed(42)  # Für Reproduzierbarkeit
    eval_indices = np.random.choice(range(min_idx, max_idx), size=min(num_samples, max_idx - min_idx), replace=False)
    eval_indices.sort()  # Sortiere für besser nachvollziehbare Ausgaben
    selected_eval_indices = [int(i) for i in eval_indices.tolist()]  # JSON-serialisierbar
    selected_eval_times = [np.datetime_as_string(times[i]) for i in selected_eval_indices]
    
    # Evaluiere jeden ausgewählten Zeitpunkt
    all_results = []
    avg_mse_values = []
    evaluation_times = []
    
    # Für die Extremwert-Plots - speichere Predictions/Targets
    all_predictions = []
    all_targets = []
    all_target_times = []
    
    # NEU: Sammle MSE pro Kanal für alle Evaluierungen
    # Kanalnamen generieren
    channel_names = []
    channel_names.extend(SINGLE_VARS)
    for level in PRESSURE_LEVELS:
        for var in PRESSURE_VARS:
    ...
    num_channels = len(SINGLE_VARS) + len(PRESSURE_VARS) * len(PRESSURE_LEVELS)
    all_mse_per_channel = np.zeros((0, num_channels))  # [n_evals, n_channels]
    
    # Modell-Vorhersagelänge einstellen
    model.set_model_prediction_length(prediction_steps)
    ...
    for i, eval_idx in enumerate(eval_indices):
        ...
        print(f"\n[INFO] Evaluierung {i+1}/{len(eval_indices)}: {eval_datetime}")
        
        # Extrahiere Kontext- und Zieldaten für diesen Zeitpunkt
        ...
        print(f"[INFO] Kontext: {len(context_times)} Zeitschritte von {np.datetime_as_string(context_times[0])} bis {np.datetime_as_string(context_times[-1])}")
        print(f"[INFO] Ziel: {len(target_times)} Zeitschritte von {np.datetime_as_string(target_times[0])} bis {np.datetime_as_string(target_times[-1])}")
        
        # Bereite Daten für das Modell vor
        x_input = torch.from_numpy(context_data).float().permute(0, 3, 1, 2).unsqueeze(0).to(device)  # [1,T_ctx,C,H,W]
        x_target = torch.from_numpy(target_data).float().permute(0, 3, 1, 2).unsqueeze(0).to(device)  # [1,T_fut,C,H,W]
        
        # Zeit-Features
        t_ctx_np = build_time_scalars(context_times)  # [T_ctx,2]
        t_fut_np = build_time_scalars(target_times)   # [T_fut,2]
        t_ctx = torch.from_numpy(t_ctx_np).float().unsqueeze(0).to(device)  # [1,T_ctx,2]
        t_fut = torch.from_numpy(t_fut_np).float().unsqueeze(0).to(device)  # [1,T_fut,2]
        ...
        evaluation_time = end_time - start_time
        print(f"[INFO] Vorhersage in {evaluation_time:.2f} Sekunden durchgeführt")
        
        # ===== CPU-Kopien anlegen und Metriken AUF CPU berechnen =====
        predictions_cpu = predictions.detach().to("cpu", non_blocking=True)  # [1,T,C,H,W]
        x_target_cpu    = x_target.detach().to("cpu", non_blocking=True)     # [1,T,C,H,W]

        # Fehlermetriken (CPU)
        se = (predictions_cpu - x_target_cpu) ** 2        # [1,T,C,H,W]
        mse_map = se.mean(dim=1)                           # [1,C,H,W]  (über Zeit)
        avg_mse = mse_map.mean().item()                    # Skalar

        mse_per_step = [
            F.mse_loss(predictions_cpu[:, t], x_target_cpu[:, t]).item()
            for t in range(len(target_times))
        ]

        # MSE pro Kanal (normalisiert, Zeit & Raum gemittelt) -> [C]
        mse_per_channel_norm_torch = se.mean(dim=(0, 1, 3, 4))  # [C]
        mse_per_channel_norm = mse_per_channel_norm_torch.numpy()

        # Für Extremfall-Plots die CPU-Kopien behalten
        all_predictions.append(predictions_cpu)
        all_targets.append(x_target_cpu)
        all_target_times.append(target_times)

        # ===== Jetzt GPU-Referenzen freigeben =====
        del predictions, x_target, x_input, t_ctx, t_fut
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        import gc; gc.collect()

        # NEU: Sammle MSE pro Kanal für diese Evaluierung
        all_mse_per_channel = np.vstack([all_mse_per_channel, mse_per_channel_norm])

        
        print(f"[INFO] MSE für {eval_datetime}: {avg_mse:.6f}")
        print(f"[INFO] MSE pro Schritt: min={min(mse_per_step):.6f}, max={max(mse_per_step):.6f}")
        
        # Erstelle Ergebnisdict für diesen Zeitpunkt
        mse_time_series = [
            {
                "step": i,
                "mse": mse,
    ...
    if all_results:
        avg_mse_mean = np.mean(avg_mse_values)
        avg_mse_std = np.std(avg_mse_values)
        avg_mse_min = np.min(avg_mse_values)
        avg_mse_max = np.max(avg_mse_values)
        avg_eval_time = np.mean(evaluation_times)
        
        # Finde Indizes für die extremen MSE-Werte
        best_idx = int(np.argmin(avg_mse_values))
        worst_idx = int(np.argmax(avg_mse_values))
        
        # Erstelle Plots für die besten und schlechtesten Fälle, wenn Logger verfügbar
        if logger and len(avg_mse_values) > 1 and visualize:
            # Zentrale Pixel-Koordinaten
            if (lat is not None) and (lon is not None):
                H, W = len(lat), len(lon)
            else:
                # Fallback: aus einer gespeicherten Prediction ableiten
                _sample_pred = all_predictions[best_idx].squeeze(0)  # [T, C, H, W]
                H, W = _sample_pred.shape[-2], _sample_pred.shape[-1]

            center_h = H // 2
            center_w = W // 2

            
            # Erstelle Plot für den besten Fall (niedrigster MSE)
            best_pred = all_predictions[best_idx].squeeze(0)  # [T, C, H, W]
            best_target = all_targets[best_idx].squeeze(0)    # [T, C, H, W]
            best_times = all_target_times[best_idx]
            best_datetime = pd.Timestamp(best_times[0]).to_pydatetime()
            
            # Erstelle Plot für den schlechtesten Fall (höchster MSE)
            worst_pred = all_predictions[worst_idx].squeeze(0)  # [T, C, H, W]
            worst_target = all_targets[worst_idx].squeeze(0)    # [T, C, H, W]
            worst_times = all_target_times[worst_idx]
            worst_datetime = pd.Timestamp(worst_times[0]).to_pydatetime()
            
            # Denormalisiere die Daten für bessere Visualisierung
            if scaler is not None:
                # Best case
                best_pred_np = best_pred.numpy()
                best_target_np = best_target.numpy()
                best_pred_hwc = best_pred_np.transpose(0, 2, 3, 1)  # [T, H, W, C]
                best_target_hwc = best_target_np.transpose(0, 2, 3, 1)  # [T, H, W, C]
                best_pred_hwc = invert_channel_scaling(best_pred_hwc, scaler)
                best_target_hwc = invert_channel_scaling(best_target_hwc, scaler)
                best_pred_np = best_pred_hwc.transpose(0, 3, 1, 2)  # [T, C, H, W]
                best_target_np = best_target_hwc.transpose(0, 3, 1, 2)  # [T, C, H, W]
                
                # Worst case
                worst_pred_np = worst_pred.numpy()
                worst_target_np = worst_target.numpy()
                worst_pred_hwc = worst_pred_np.transpose(0, 2, 3, 1)  # [T, H, W, C]
                worst_target_hwc = worst_target_np.transpose(0, 2, 3, 1)  # [T, H, W, C]
                worst_pred_hwc = invert_channel_scaling(worst_pred_hwc, scaler)
                worst_target_hwc = invert_channel_scaling(worst_target_hwc, scaler)
                worst_pred_np = worst_pred_hwc.transpose(0, 3, 1, 2)  # [T, C, H, W]
                worst_target_np = worst_target_hwc.transpose(0, 3, 1, 2)  # [T, C, H, W]
            else:
                best_pred_np = best_pred.numpy()
                best_target_np = best_target.numpy()
                worst_pred_np = worst_pred.numpy()
            ...
            # Konvertiere Zeitstempel für die x-Achse
            best_dt_times = [pd.Timestamp(t).to_pydatetime() for t in best_times]
            worst_dt_times = [pd.Timestamp(t).to_pydatetime() for t in worst_times]
            
            # Erstelle die Plots für alle Temperaturkanäle
            temp_channels = [i for i, ch in enumerate(channel_names) if ch == "t2m" or ch.startswith("t_")]
            for c in temp_channels:
                ch = channel_names[c]
                
                # Plot für besten Fall
                fig_best, ax_best = plt.subplots(figsize=(12, 6))
                ax_best.plot(best_dt_times, best_pred_np[:, c, center_h, center_w], "-o", label="Vorhersage")
                ax_best.plot(best_dt_times, best_target_np[:, c, center_h, center_w], "-x", label="Ground Truth")
                ax_best.set_title(f"Bester Fall: {ch} am zentralen Pixel - {best_datetime.strftime('%Y-%m-%d %H:%M')} (MSE: {avg_mse_values[best_idx]:.6f})")
                ax_best.set_xlabel("Zeit")
                ax_best.set_ylabel("Temperatur [K]")
                ax_best.grid(True)
                ax_best.legend()
                ax_best.xaxis.set_major_formatter(mdates.DateFormatter("%d.%m %H:%M"))
                plt.setp(ax_best.xaxis.get_majorticklabels(), rotation=45)
                plt.tight_layout()
                
                # Plot für schlechtesten Fall
                fig_worst, ax_worst = plt.subplots(figsize=(12, 6))
                ax_worst.plot(worst_dt_times, worst_pred_np[:, c, center_h, center_w], "-o", label="Vorhersage")
                ax_worst.plot(worst_dt_times, worst_target_np[:, c, center_h, center_w], "-x", label="Ground Truth")
                ax_worst.set_title(f"Schlechtester Fall: {ch} am zentralen Pixel - {worst_datetime.strftime('%Y-%m-%d %H:%M')} (MSE: {avg_mse_values[worst_idx]:.6f})")
                ax_worst.set_xlabel("Zeit")
                ax_worst.set_ylabel("Temperatur [K]")
                ax_worst.grid(True)
                ax_worst.legend()
                ax_worst.xaxis.set_major_formatter(mdates.DateFormatter("%d.%m %H:%M"))
                plt.setp(ax_worst.xaxis.get_majorticklabels(), rotation=45)
                plt.tight_layout()
                
                # Log zu WandB
                logger.experiment.log({
                    f"extreme_cases/{ch}_best_mse_{avg_mse_values[best_idx]:.4f}": wandb.Image(fig_best),
                    f"extreme_cases/{ch}_worst_mse_{avg_mse_values[worst_idx]:.4f}": wandb.Image(fig_worst)
                })
                
                plt.close(fig_best)
                plt.close(fig_worst)
        
        # Berechne durchschnittlichen MSE pro Kanal
        avg_mse_per_channel = np.mean(all_mse_per_channel, axis=0)  # [C]
        
        # Logge Zusammenfassung zu W&B
        if logger:
            metrics = {
            ...
            # NEU: Logge MSE pro Kanal (Durchschnitt über alle Evaluierungen)
            for i, ch in enumerate(channel_names):
        ...
        summary = {
            "avg_mse_mean": float(avg_mse_mean),
            "avg_mse_std": float(avg_mse_std),
            "avg_mse_min": float(avg_mse_min),
            "avg_mse_max": float(avg_mse_max),
            ...
            # NEU: Füge MSE pro Kanal zur Zusammenfassung hinzu
        ...
        print(f"\n[INFO] Zusammenfassung der Evaluierung über {len(all_results)} Zeitpunkte:")
        print(f"  Durchschnittliche MSE:   {avg_mse_mean:.6f} ± {avg_mse_std:.6f}")
        print(f"  MSE-Bereich:             {avg_mse_min:.6f} - {avg_mse_max:.6f}")
        print(f"  Durchschn. Eval-Zeit:    {avg_eval_time:.2f}s")
        
        # NEU: Ausgabe der Top 3 Kanäle mit höchstem MSE
        ...
        print("\n[INFO] Top 3 Kanäle mit höchstem MSE:")
        for ch, mse in mse_channel_pairs[:3]:
            print(f"  {ch}: {mse:.6f}")
        
        return summary
    else:
        print("[WARNING] Keine erfolgreichen Evaluierungen durchgeführt")
        return {
            "error": "Keine erfolgreichen Evaluierungen durchgeführt",
            "num_samples": 0
        }
...
def train_model(train_loader, val_loader, in_channels, hidden_dim=64, num_heads=4, 
    ...
    print(f"[INFO] Setze Vorhersagehorizont auf {prediction_steps} Zeitschritte")
    model.set_model_prediction_length(prediction_steps)
...
def load_model(model_path=None, version=None, model_name=None, use_axial_attention: bool = False):
    """
...
def main():
    ...
    parser.add_argument('--context_days_list', type=str, 
                       help='Kommagetrennte Liste von Kontexttagen für die Evaluierung (z.B. "0.5,1,2")')
    ...
    parser.add_argument('--use_flash_attention', type=lambda x: x.lower()=="true", default=True, help='Use Flash Attention if available')
    ...
    parser.add_argument("--use_axial_attention", action="store_true", default=False, help="Use axial self-attention variant of the weather transformer.")
    ### Fine-tuning 
    ...
    if args.evaluate_random:
        ...
        # Bestimme Liste der zu evaluierenden Kontextlängen
        if args.context_days_list:
            context_days_list = [int(x) for x in args.context_days_list.split(',')]
            print(f"[INFO] Evaluiere mit {len(context_days_list)} verschiedenen Kontextlängen: {context_days_list}")
        else:
            context_days_list = [int(args.context_days)]  # Standardwert
        ...
        # Lade alle Daten für dieses Jahr
        print("[INFO] Lade alle Daten für das Jahr (dies kann einige Zeit dauern)...")
        times, lat, lon, X = build_stacked_tensor_single_plus_pressure(single_paths, pressure_paths)
        ...
        print("[INFO] Normalisiere Daten...")
        X_normalized = apply_channel_scaling(X, model.scaler)
        
        # Schleife über alle Kontextlängen
        for context_days in context_days_list:
            current_context_len = int(context_days * HOURS_IN_DAY)
            print(f"\n[INFO] Evaluiere mit context_days={context_days} (context_len={current_context_len})")
            
            results = evaluate_random_dates(
                model=model,
                num_samples=args.evaluate_random,
                context_len=current_context_len,  # Verwende aktuelle Kontextlänge
                prediction_steps=prediction_steps,
                logger=logger,
                times=times,                      # Übergebe vorgeladene Daten
                X_normalized=X_normalized,
                lat=lat,
                lon=lon
            )

            # Ergebnisse speichern mit Kontextlänge im Dateinamen
            version = args.version or "custom"
            base_dir = os.path.join("weather_prediction", "forecast_results")
            os.makedirs(base_dir, exist_ok=True)

            results_file = os.path.join(base_dir, f"random_eval_{PREDICTION_YEAR}_n{args.evaluate_random}_v{version}_c{context_days}.json")
            with open(results_file, 'w') as f:
                json.dump(results, f, indent=4)

            print(f"[INFO] Evaluierungsergebnisse gespeichert in: {results_file}")
